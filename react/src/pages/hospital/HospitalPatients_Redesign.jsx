      ...
      {data.map((d) => (
        <div key={d.label} className="space-y-1">
          <div className="flex items-center justify-between text-xs">
            <span className="font-medium text-slate-700 dark:text-slate-200">{d.label} yrs</span>
            <span className="font-semibold text-slate-900 dark:text-slate-100 tabular-nums">{d.pct}%</span>
          </div>
...
export default function HospitalPatients() {
  ...
  const getStatusClass = (status) => {
    if (status === 'stable') return 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border-emerald-500/20';
    if (status === 'recovering') return 'bg-sky-500/10 text-sky-700 dark:text-sky-300 border-sky-500/20';
    if (status === 'critical') return 'bg-red-500/10 text-red-700 dark:text-red-300 border-red-500/20';
  ...
  return (
    <div className="min-h-screen -m-6 p-6 bg-gradient-to-br from-slate-50 via-sky-50 to-slate-50 dark:from-slate-950 dark:via-slate-950 dark:to-slate-950">
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between mb-6">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Patient Management</h1>
          <p className="text-sm md:text-base text-slate-600 dark:text-slate-300 mt-1">Active patient registry</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
            <input type="text" placeholder="Search patients..." className="pl-9 pr-4 py-2 rounded-2xl bg-white/40 dark:bg-white/5 border border-white/50 dark:border-white/10 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500/50 backdrop-blur-xl" />
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <GlassCard className="lg:col-span-2">
          <div className="p-5 border-b border-white/20">
            <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">Patient growth trend</div>
            <div className="text-xs text-slate-600 dark:text-slate-300 mt-1">Monthly patient admissions</div>
          </div>
          <div className="p-5">
            <PatientGrowthChart />
          </div>
        ...
        <GlassCard className="p-5">
          <div className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-4">Age distribution</div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/20">
                ...
                <th className="px-5 py-4 text-left text-xs font-semibold text-slate-900 dark:text-slate-100">Age</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-slate-900 dark:text-slate-100">Type</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-slate-900 dark:text-slate-100">Department</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-slate-900 dark:text-slate-100">Room</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-slate-900 dark:text-slate-100">Status</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-slate-900 dark:text-slate-100">Doctor</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-slate-900 dark:text-slate-100">Admitted</th>
                <th className="px-5 py-4 text-right text-xs font-semibold text-slate-900 dark:text-slate-100">Actions</th>
              </tr>
            </thead>
            <tbody>
              {patients.map((patient) => (
                <tr key={patient.id} className="border-b border-white/10 hover:bg-white/20 dark:hover:bg-white/5 transition-all">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-sky-400/30 to-indigo-400/30 border border-white/50 dark:border-white/10 flex items-center justify-center">
                        <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{patient.name[0]}</span>
                      </div>
                      <div>
                        <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">{patient.name}</div>
                        <div className="text-xs text-slate-600 dark:text-slate-300">{patient.gender === 'M' ? 'Male' : 'Female'}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-4 text-sm text-slate-700 dark:text-slate-200 tabular-nums">{patient.age}</td>
                  <td className="px-5 py-4">
                    <span className={`inline-flex px-2 py-1 rounded-xl text-[11px] font-semibold ${patient.type === 'in-patient' ? 'bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 border border-indigo-500/20' : 'bg-teal-500/10 text-teal-700 dark:text-teal-300 border border-teal-500/20'}`}>{patient.type}</span>
                  </td>
                  <td className="px-5 py-4 text-sm text-slate-700 dark:text-slate-200">{patient.dept}</td>
                  <td className="px-5 py-4 text-sm text-slate-700 dark:text-slate-200 tabular-nums font-mono">{patient.room}</td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4 text-sm text-slate-700 dark:text-slate-200">{patient.doctor}</td>
                  <td className="px-5 py-4 text-xs text-slate-600 dark:text-slate-300">{patient.admitted}</td>
