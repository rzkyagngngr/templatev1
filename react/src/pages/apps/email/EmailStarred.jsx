  ...
  return (
    <div className="w-full p-6 space-y-6 bg-gradient-to-br from-slate-50 via-white to-slate-100 min-h-screen">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h4 className="text-3xl font-bold bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent">Starred Emails</h4>
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <input
              ...
              className="pl-10 pr-4 py-2 w-64 rounded-2xl border border-white/60 bg-white/70 backdrop-blur-2xl shadow-lg focus:outline-none focus:ring-2 focus:ring-yellow-400/50 focus:border-yellow-400/60 transition-all duration-300"
            />
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-3">
          {/* Compose Button */}
          <button className="w-full mb-6 px-6 py-3 bg-gradient-to-r from-yellow-400 to-orange-500 hover:from-yellow-500 hover:to-orange-600 text-white rounded-2xl font-semibold flex items-center justify-center gap-2 shadow-lg shadow-yellow-400/25 hover:shadow-yellow-400/40 transition-all duration-300 hover:scale-105">
          ...
          <div className="glass-card bg-white/80 backdrop-blur-2xl border border-white/60 shadow-2xl rounded-3xl p-6 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <a href="/apps/email/starred" className="flex items-center gap-3 p-3 bg-gradient-to-r from-yellow-400/20 to-orange-400/20 text-yellow-700 rounded-2xl text-sm font-semibold shadow-lg border border-yellow-400/30">
                <Star className="w-4 h-4 fill-yellow-500 text-yellow-600" />
                Starred
                <span className="ml-auto bg-yellow-500 text-white px-2 py-1 rounded-full text-xs">{emails.length}</span>
              </a>
        ...
        <div className="lg:col-span-9">
          ...
          <div className="glass-card bg-white/80 backdrop-blur-2xl border border-white/60 shadow-2xl rounded-3xl overflow-hidden">
            {emails.map((email, idx) => (
              <div
                key={idx}
                className={`p-4 border-b border-white/30 hover:bg-gradient-to-r hover:from-yellow-400/10 hover:to-orange-400/10 transition-all duration-300 cursor-pointer ${
                  !email.read ? 'bg-gradient-to-r from-yellow-400/5 to-orange-400/5' : ''
                }`}
              >
                <div className="flex items-center gap-3">
                  <input
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2 mb-1">
                      <span className={`font-semibold text-sm ${!email.read ? 'text-slate-900' : 'text-slate-700'}`}>
                        {email.sender}
                      </span>
                      {email.label && (
                        <span className={`text-xs font-semibold px-3 py-1 rounded-full border ${email.labelColor} shadow-sm`}>
                          {email.label}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`font-semibold text-sm ${!email.read ? 'text-slate-900' : 'text-slate-700'} truncate`}>
                        {email.subject}
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    >
                      <Star className={`w-5 h-5 ${email.starred ? 'fill-yellow-400 text-yellow-500' : 'text-slate-400'} hover:text-yellow-400 transition-colors`} />
                    </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
            <div className="glass-card bg-gradient-to-br from-yellow-400/20 to-orange-400/20 backdrop-blur-2xl border border-yellow-400/30 shadow-lg rounded-2xl p-4 text-center">
              <div className="text-2xl font-bold text-yellow-700">{emails.length}</div>
              <div className="text-sm text-yellow-600">Starred Emails</div>
            </div>
            <div className="glass-card bg-gradient-to-br from-blue-400/20 to-purple-400/20 backdrop-blur-2xl border border-blue-400/30 shadow-lg rounded-2xl p-4 text-center">
              <div className="text-2xl font-bold text-blue-700">{emails.filter(e => !e.read).length}</div>
              <div className="text-sm text-blue-600">Unread</div>
            </div>
            <div className="glass-card bg-gradient-to-br from-green-400/20 to-teal-400/20 backdrop-blur-2xl border border-green-400/30 shadow-lg rounded-2xl p-4 text-center">
              <div className="text-2xl font-bold text-green-700">{emails.filter(e => e.read).length}</div>
